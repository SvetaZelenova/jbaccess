...
export class ControllersService {
  ...
  getDoorsByControllerId(id: number): Observable<Door[]> {
    return this.http.get<ApiResponse<Door[]>>(this.controllerPath + id + '/doors')
      .pipe(
  ...
  updateDoorRelation(relation: Relation): Observable<boolean> {
    const url = this.controllerPath + relation.parentId + '/doors/' + relation.relatedEntityId;
  ...
  getDoorsRelationsByControllerId(id: number): Observable<Relation[]> {
    return zip(this.doorsService.getAllDoors(), this.getDoorsByControllerId(id))
      .pipe(
        map(data => {
        ...
        catchError(this.handleError<Relation[]>('getDoorsRelationsByControllerId', []))
      );
  }
