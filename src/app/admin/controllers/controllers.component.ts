...
export class ControllersComponent implements OnInit {
  ...
  editDoors(controller: Controller) {
    ...
    this.controllersService
      .getDoorsRelationsByControllerId(controller.id)
      .subscribe(rel => {
        this.doors = rel;
      });
  ...
  updateDoorRelation(newRelation: Relation) {
    this.controllersService.updateDoorRelation(newRelation).subscribe(
      () =>
  ...
  loadControllers(showMessage: boolean = true) {
    ...
    this.controllersService.getAllControllers().subscribe(data => {
  ...
  displayControllerForm(controller?: Controller) {
    ...
    const controllerForm = controller
      ? controller
      : { id: 0, name: '', controllerId: '' };
  ...
  createController(controller: Controller) {
    this.controllersService.createController(controller).subscribe(
      updatedController => {
        this.loadControllers(false);
  ...
  updateController(controller: Controller) {
    this.controllersService.updateController(controller).subscribe(
      updatedController => {
        this.loadControllers(false);
        this.hideControllerForm();
        this.messageService.add({
