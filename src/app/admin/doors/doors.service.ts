...
export class DoorsService {
  ...
  getAllDoors(): Observable<Door[]> {
    return this.http
      .get<ApiResponse<Door[]>>(this.doorPath)
      .pipe(map(res => res.payload));
  }
  createDoor(door: Door): Observable<Door> {
    return this.http.post<ApiResponse<Door>>(this.doorPath, door).pipe(
      map(data => ({
        id: data.payload.id,
  ...
  updateDoor(door: Door): Observable<Door> {
    return this.http.put<ApiResponse<Door>>(this.doorPath + door.id, door).pipe(
      map(data => ({
        id: data.payload.id,
        name: data.payload.name,
        accessId: data.payload.accessId
  ...
  deleteDoor(id: number): Observable<number> {
    return this.http.delete<ApiResponse<any>>(this.doorPath + id).pipe(
