...
export class RolesService {
  ...
  getAllRoles(): Observable<{roles: Role[]}> {
    return this.http.get<ApiResponse<Role[]>>(this.rolePath)
      .pipe(
        map((data) => {
  ...
  createRole(role: Role): Observable<Role> {
    return this.http.post<ApiResponse<Role>>(this.rolePath, role)
      .pipe(
        map( data => ({
          id: data.payload.id,
  ...
  updateRole(role: Role): Observable<Role> {
    return this.http.put<ApiResponse<Role>>(this.rolePath + role.id, role)
      .pipe(
        map(data => ({
          id: data.payload.id,
          name: data.payload.name})),
  ...
  deleteRole(id: number): Observable<number> {
    return this.http.delete<ApiResponse<any>>(this.rolePath + id)
      .pipe(
